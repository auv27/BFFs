...
pacman::p_load(tidyverse, psych, Hmisc, mice, miceadds, lmerTest, broom.mixed, 
               partR2, sjPlot, viridis, patchwork, parameters)
...
summary(mlm1s)
tab_model(mlm1s, digits = 3, show.se = TRUE, show.stat = TRUE, show.ci = FALSE)
...
int1 <- plot_model(mlm1s, type = "pred", 
  ...
  ggtitle("") + 
  labs(y = "Inter-Brain Neural Similarity",
...
## Reviewers requested standardized betas
```{r}
# Primary model
mlm1i_standardizedbetas_list <- lapply(1:25, function(i) {
...
combined_df <- bind_rows(mlm1i_standardizedbetas_list)

combined_df |>
  group_by(Parameter) |>
  summarise(
    beta_mean = mean(Std_Coefficient),
    beta_sd = sd(Std_Coefficient),
    .groups = "drop"
  )

# Models looking at region specific effects
## dmpfc
dmpfc_standardizedbetas_list <- lapply(1:25, function(i) {
  
  model <- lmer(deconMeanCor ~ traitRatings + 
                stateRatings*neuralSimConditionCodes*saTotalC + 
                (stateRatings | dyad), data = dmPFC_data[[i]])
  
  data.frame(model = i, as.data.frame(standardize_parameters(model, method = "refit")))
})

combined_df <- bind_rows(dmpfc_standardizedbetas_list)

combined_df |>
  group_by(Parameter) |>
  summarise(beta_mean = mean(Std_Coefficient),
            beta_sd = sd(Std_Coefficient),
            .groups = "drop")

### random effects 
dmpfc_random_effects_list <- lapply(1:25, function(i) {
  model <- lmer(deconMeanCor ~ traitRatings + 
                stateRatings*neuralSimConditionCodes*saTotalC + 
                (stateRatings | dyad), data = dmPFC_data[[i]])
...
combined_df <- bind_rows(dmpfc_random_effects_list) |>
  unite(item, c("grp", "var1", "var2")) |>
  mutate(item = str_replace_all(item, fixed("NA"), ""),
         item = str_replace_all(item, fixed("("), ""),
         item = str_replace_all(item, fixed(")"), ""),
...
## rtpj
rtpj_standardizedbetas_list <- lapply(1:25, function(i) {
  
  model <- lmer(deconMeanCor ~ traitRatings + 
                stateRatings*neuralSimConditionCodes*saTotalC + 
                (stateRatings | dyad), data = rTPJ_data[[i]])
  
  data.frame(model = i, as.data.frame(standardize_parameters(model, method = "refit")))
})

combined_df <- bind_rows(rtpj_standardizedbetas_list)
...
### random effects 
rtpj_random_effects_list <- lapply(1:25, function(i) {
  model <- lmer(deconMeanCor ~ traitRatings + 
                stateRatings*neuralSimConditionCodes*saTotalC + 
                (stateRatings | dyad), data = rTPJ_data[[i]])
...
combined_df <- bind_rows(rtpj_random_effects_list) |>
  unite(item, c("grp", "var1", "var2")) |>
  mutate(item = str_replace_all(item, fixed("NA"), ""),
         item = str_replace_all(item, fixed("("), ""),
         item = str_replace_all(item, fixed(")"), ""),
