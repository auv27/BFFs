...
read.csv("~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_32/coding/dyad_32_behavior.csv", stringsAsFactors = FALSE) %>%
  ...
  mutate(id1 = "2337_32_63", id2 = "2337_32_64") %>%
...
#df %>%
  #filter(epoch == 16 | epoch == 17 | epoch == 27 | epoch == 28) %>%
...
neg_pos_neu %>%
  ...
  sample_n(., 3) %>%
  mutate(emotion = "neu",
         clip = row_number()) -> neu_add1

df %>%
  filter(epoch == 13 | epoch == 54) %>%
  dplyr::select(id1, id2, onset, offset, selectedAffect, relativeAffect) -> neu_add2
...
Reduce(function(...) {full_join(...)}, list(neg, pos, neu)) %>%
  arrange(onset) %>%
  mutate(fileType = ".mp4",
         filePath = "subject_files/dyad_32/clips/") -> all_clips

write.csv(all_clips, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_32/coding/dyad_32_selectedAffect.csv")
```
...
write.csv(epochs, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_32/coding/dyad_32_epochs.csv")
...
write.csv(id1_random, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_32/63_epochs.csv", row.names = FALSE)
...
write.csv(id2_random, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_32/64_epochs.csv", row.names = FALSE)
```
