...
read.csv("~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_16/coding/dyad_16_behavior.csv", stringsAsFactors = FALSE) %>%
  ...
  mutate(id1 = "2337_16_31", id2 = "2337_16_32") %>%
...
neg_pos_neu %>%
  filter(selectedAffect == 0 & relativeAffect == -.5 | selectedAffect == 0 & relativeAffect == .5) %>% 
  sample_n(., 6) %>%
  mutate(emotion = "neu",
         clip = row_number())-> neu
...
Reduce(function(...) {full_join(...)}, list(neg, pos, neu)) %>%
  arrange(onset) %>%
  mutate(fileType = ".mp4",
         filePath = "subject_files/dyad_16/clips/") -> all_clips

write.csv(all_clips, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_16/coding/dyad_16_selectedAffect.csv")
```
...
write.csv(epochs, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_16/coding/dyad_16_epochs.csv")
...
write.csv(id1_random, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_16/31_epochs.csv", row.names = FALSE)
...
write.csv(id2_random, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_16/32_epochs.csv", row.names = FALSE)
```
