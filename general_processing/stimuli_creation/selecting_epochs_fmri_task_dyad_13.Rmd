...
read.csv("~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_13/coding/dyad_13_behavior.csv", stringsAsFactors = FALSE) %>%
  ...
  mutate(id1 = "2337_13_25", id2 = "2337_13_26") %>%
...
df %>%
  ...
  mutate(selectedSegments = 1) -> neg_add
...
df %>%
  filter(epoch == 4 | epoch == 5 | epoch == 18 | epoch == 38 | epoch == 49) %>%
  dplyr::select(id1, id2, onset, offset, selectedAffect, relativeAffect) %>%
  mutate(selectedSegments = 1) -> neu_add
...
Reduce(function(...) {full_join(...)}, list(neg, pos, neu)) %>%
  arrange(onset) %>%
  mutate(fileType = ".mp4",
         filePath = "subject_files/dyad_11/clips/") -> all_clips

write.csv(all_clips, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_13/coding/dyad_13_selectedAffect.csv")
```
...
write.csv(epochs, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_13/coding/dyad_13_epochs.csv")
...
write.csv(id1_random, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_13/25_epochs.csv", row.names = FALSE)
...
write.csv(id2_random, "~/Library/CloudStorage/OneDrive-ThePennsylvaniaStateUniversity/Research Projects/F31/BFFs/data_processing/stimuli_creation/tpv_13/26_epochs.csv", row.names = FALSE)
```
